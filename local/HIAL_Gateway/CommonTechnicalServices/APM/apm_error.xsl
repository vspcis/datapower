...
<!--
  ...
  3. Update the Fault/faultcode and Fault/faultstring text
     to indicate where the failure was.   
...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:variable name="filename" select="'apm_error.xsl'"/>
    ...
    <!-- Error rule specific handlers to rewrite the SOAP Body as a valid fault.
         DataPower has a habit of returning the original request back as an error response.  -->
    
    <xsl:template match="soap11:Envelope/soap11:Body">
        <xsl:message><xsl:value-of select="($filename)"/>: Rebuilding SOAP 1.1 Body as a fault</xsl:message>
        <xsl:copy>
            <xsl:element name="soap11:Fault">
                <xsl:element name="soap11:faultcode">
                    <xsl:choose>
                        <xsl:when test="$rmode='2'">soap:Server</xsl:when>
                        <xsl:otherwise>soap:Client</xsl:otherwise>
                    </xsl:choose>
                </xsl:element>
                <xsl:element name="soap11:faultstring">
                    <xsl:choose>
                        <xsl:when test="$rmode='2'">Internal Error (from server)</xsl:when>
                        <xsl:otherwise>Internal Error (from client)</xsl:otherwise>
                    </xsl:choose>
                </xsl:element>
            </xsl:element> <!-- end SOAP 1.1 Fault -->
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="soap12:Envelope/soap12:Body">
        <xsl:message><xsl:value-of select="($filename)"/>: Rebuilding SOAP 1.2 Body as a fault</xsl:message>
        <xsl:copy>
        <!--  do we need to use namespace-uri() to fill in the namespace here somehow? -->
            <!-- SOAP 1.2 fault -->
            <xsl:element name="soap12:Fault">
                <xsl:element name="soap12:Code">
                    <xsl:element name="soap12:Value">
                        <xsl:choose>
                            <xsl:when test="$rmode='2'">soap:Receiver</xsl:when>
                            <xsl:otherwise>soap:Sender</xsl:otherwise>
                        </xsl:choose>
                    </xsl:element>
                </xsl:element>
                <xsl:element name="soap12:Reason">
                    <xsl:element name="soap12:Text">
            ...
            </xsl:element> <!-- end SOAP 1.2 Fault -->
        </xsl:copy>
    </xsl:template>
